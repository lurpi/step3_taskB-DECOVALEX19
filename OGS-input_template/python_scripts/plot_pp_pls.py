...
r1="..\\taskB-step3-2d_time_MONITORING2.tec"
r2="..\\taskB-step3-2d_time_MONITORING3.tec"
r3="..\\taskB-step3-2d_time_MP_18above.tec"
...
dS2=[]
dS_f2=[]
# create time array for failures
Xf2=[]
Xf_f2=[]
Xf1=[]
#
#for i in range(len(S1)-1):    ## to evaluate when new plastic strain
...
#    dS_f2.append(tmp_f2) 
#    Xf_f2.append(X_f2[i])
for i in range(len(S1)-1): ## start plotting only if plastic strain not 0
 tmp=S1[i]
 if tmp==0:
     tmp=' '
 else:    
...
ax1.plot(X2, P2/1.e6, 'b-', lw=2.5,label='Pressure at P3 (below)')
ax1.plot(X_f2, P_f2/1.e6, 'g-', lw=2.5,label='Pressure at 18cm below')
ax1.plot(X1, P1/1.e6, 'r-', lw=3.5,label='Pressure at P2 (strike)')

ax2.plot(Xf2[:-1], dS2[:-1], 'bo', lw=2.5,label='Plastic shear displacement at P3 (below)')
ax2.plot(Xf_f2[:-1], dS_f2[:-1], 'g*', lw=2.5,label='Plastic shear displacement at 18cm below')
ax2.plot(Xf1[:-1], dS1[:-1], 'r+', lw=5.5,label='Plastic shear displacement at P2 (strike)')
...
ax1.set_xlim(0,1000)
ax1.set_ylim(0,7)
ax2.set_ylabel('Plastic shear displacement (m)', fontsize=28)
